...
// ---------- planet + moon data (key moons only) ----------
const planetSystems = [
  {
    name: 'Earth',
    color: 0x4fa7ff,          // deep blue
    radius: 120,
    orbitalSpeed: 1.0,
    tiltPhase: 0.4,
    moons: [
      { name: 'Moon', color: 0xfafafa, r: 26, speed: 12.0 }
    ]
  },
  {
    name: 'Mars',
    color: 0xd2553f,          // rusty red
    radius: 160,
    orbitalSpeed: 0.53,
    tiltPhase: 1.2,
    moons: [
...
const trailLength = 4000; // longer, smoother flowers
...
function update(dt) {
  ...
  systems.forEach((sys, si) => {
    ...
    // planet motion around Sun (ellipse + wobble)
    ...
    sys.moons.forEach((moon, mi) => {
      ...
      // radial modulation to create petals
      const lobes = 5 + mi * 2; // 5,7,9 petals...
      const radialMod = 1.0 + 0.18 * Math.sin(lobes * moon.angle + si * 0.7);

      const mxLocal = Math.cos(moon.angle) * md.r * radialMod;
      const mzLocal = Math.sin(moon.angle) * md.r * radialMod;
      const myLocal = Math.sin(moon.angle * 0.7 + localTilt) * (md.r * 0.25);
...
function animate() {
  ...
  // long‑exposure style fade
  renderer.setClearColor(0x000000, 0.035);
  renderer.clearColor();
